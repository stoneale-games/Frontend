...
interface FrequencyHistogramMetaData {
	// Define properties based on how it's used
	[key: string]: any;
}
...
type PushAnimationStateFn = (playerIndex: number, message: string) => void;

const handleAI = (state: GameState, pushAnimationState: PushAnimationStateFn): GameState => {
	const { highBet } = state;
	...
	switch(state.phase) {
		case('betting1'): { 
			...
			if (willCall) {
				if (willRaise(raiseChance) && raiseRange) {
					...
					if (wantRaise) {
						...
						const updatedState = handleBet(state, betValue, min, max);
						if (!updatedState.communityCards) {
							updatedState.communityCards = state.communityCards;
						}
						return updatedState;
					} else {
		...
		case('betting4'): {
			...
			} = isFlush ? checkStraightFlush(flushCards) : { isStraightFlush: false, isLowStraightFlush: false, concurrentSFCardValues: [], concurrentSFCardValuesLow: [] };
...
const generateHistogram = (hand: Card[]): Histogram => {
