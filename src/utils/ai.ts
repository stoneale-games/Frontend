...
const handleAI = (state: any, pushAnimationState: any) => {
    const { highBet } = state;
    const activePlayer = state.players[state.activePlayerIndex] as any;
    const min = determineMinBet(highBet, activePlayer.chips, activePlayer.bet) as any;
    ...
    const descendingSortHand = activePlayer.cards.concat(state.communityCards).sort((a: any, b: any) => b.value - a.value);
    ...
    const preFlopValues = activePlayer.cards.map((el: any) => el.value);
    const highCard = Math.max(...preFlopValues);
    const lowCard = Math.min(...preFlopValues);
    
    switch(state.phase) {
        case('betting1'): { 
            const suited = Object.entries(suitHistogram).find((keyValuePair: any) => keyValuePair[1] === 2);        
            ...
            if (willCall) {
                if (willRaise(raiseChance)) {
                    const determinedRaiseRange = raiseRange? raiseRange[Math.floor(Math.random() * (raiseRange.length - 0)) + 0] : ""
        ...
        case('betting4'): {
            ...
            const flushCards = (isFlush) && 
                descendingSortHand
                    .filter((card: any) => card.suit === flushedSuit);
            ...
            const highRank = handHierarchy[handHierarchy.findIndex((el: any) => el.match === true)].name;
...
const buildGeneralizedDeterminant = (hand: any, highRank: any, frequencyHistogramMetaData: any) => {
...
const buildPreFlopDeterminant = (highCard: any, lowCard: any, suited: any, straightGap: any) => {
    if (highCard === lowCard) {
        switch(highCard) {
            case(highCard > 8): {
                return {
...
const classifyStakes = (percentage: any) => {
    switch (true) {
        case (percentage > 75):
            return 'beware';
        case (percentage > 40):
...
const decideBetProportion = (stakes: any) => {
    if (stakes === 'blind') {
...
const willRaise = (chance: any) => {
    return Math.random() < chance;
}

const generateHistogram = (hand: any) => {
    const histogram = hand.reduce((acc: any, cur: any) => {
