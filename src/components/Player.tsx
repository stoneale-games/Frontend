import React from 'react';
import Card from './cards/Card';
import HiddenCard from './cards/HiddenCard';
import { Card as CardType, } from '../utils/types';
// import PlayerStatusNotificationBox from "./PlayerStatusNotificationBox";



interface PlayerProps {
  arrayIndex: number;   
  playerAnimationSwitchboard: Record<number, {isAnimating: boolean, content: string | null}>;
  endTransition: (index: number) => void;
  hasDealerChip: boolean;
  isActive: boolean;
  phase: string;
  clearCards?: boolean;
  player: {
    robot: boolean;
    folded: boolean;
    cards: CardType[];
    avatarURL: string;
    name: string;
    chips: number;
    bet: number;
  };
}

const Player: React.FC<PlayerProps> = ({
  arrayIndex,
//   playerAnimationSwitchboard,
//   endTransition,
  hasDealerChip,
  isActive,
  phase,
  clearCards,
  player: {
    robot,
    folded,
    cards,
    avatarURL,
    name,
    chips,
    bet
  }
}) => {


  const renderDealerChip = () => {
    if (!hasDealerChip) return null;
    return (
      <div className="dealer-chip-icon-container">
        {/* <img src={dealerChipImageURL} alt="Dealer Chip"/> */}c
      </div>
    );
  };

  const renderPlayerCards = () => {
    const applyFoldedClassname = folded || clearCards;

    if (robot) {
      return cards.map((card, index) => {
        if (phase !== 'showdown') {
          return (
            <HiddenCard 
              key={index} 
              cardData={card} 
              applyFoldedClassname={applyFoldedClassname}
            />
          );
        }
        const cardData = {...card, animationDelay: 0};
        return (
          <Card 
            key={index} 
            cardData={cardData} 
            applyFoldedClassname={applyFoldedClassname}
          />
        );
      });
    }

    return cards.map((card, index) => (
      <Card 
        key={index} 
        cardData={card} 
        applyFoldedClassname={applyFoldedClassname}
      />
    ));
  };

//   const ifAnimating = (playerBoxIndex: number) => {
//     return playerAnimationSwitchboard[playerBoxIndex]?.isAnimating || false;
//   };

  return (
    <div className={`player-entity--wrapper p${arrayIndex}`}>
      {/* <PlayerStatusNotificationBox
        index={arrayIndex}
        isActive={ifAnimating(arrayIndex)}
        content={playerAnimationSwitchboard[arrayIndex]?.content || null}
        endTransition={endTransition}
      /> */}
      <div className='centered-flex-row abscard'>
        {renderPlayerCards()}
      </div>
      <div className="player-entity--container">
        <div className="player-avatar--container">
          <img 
            className={`player-avatar--image${isActive ? ' activePlayer' : ''}`} 
            src={avatarURL} 
            alt="Player Avatar" 
          />
          <h5 className="player-info--name" style={{fontSize: (name.length < 14) ? 12 : 10}}>
            {name}
          </h5>
          <div className="player-info--stash--container">
            {/* <img className="player-info--stash--image" src={chipCountImageURL} alt="Player Stash"/> */}
            <h5>{chips}</h5>
          </div>
          <div className="player-info--bet--container">
            {/* <img className="player-info--bet--image" src={playerBetImageURL} alt="Player Bet" /> */}
            <h5>{`Bet: ${bet}`}</h5>
          </div>
          {renderDealerChip()}
        </div>
      </div>
    </div>
  );
};

export default Player;